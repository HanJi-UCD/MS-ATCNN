...
for i in range(Trail_times):
    ...
    results3 = start_simualtion(net, eng, Trained_Net, M, opt_mode, '200ms', 'ATCNN', 'Average')
    print('For ATCNN@200ms UTI, average throughput is:', results3[0])
    
    results4 = start_simualtion(net, eng, Trained_Net, M, opt_mode, 'AverUTI', 'ATCNN', 'Average')
    print('For ATCNN with average UTI, average throughput is:', results4[0])
    
    results5 = start_simualtion(net, eng, Trained_Net, M, opt_mode, '1000ms', 'ATCNN', 'Average')
    print('For ATCNN@1000ms UTI, average throughput is:', results5[0])
    
    toc = time.time()
    
    log = [i, net.UE_num, net.Aver_Velocity*100, results1[0], results2[0], results3[0], results4[0], results5[0], toc-tic]
        
    csv.update(log, 'result/Fig1_Thr_vs_UTI.csv')
