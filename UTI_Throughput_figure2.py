...
from mytopo import Net, start_simualtion, CsvFig2, mobility_trace
import os
import time
...
for i in range(Trail_times):
    ...
    results1 = start_simualtion(net, eng, Trained_Net, M, opt_mode, 'Dynamic', 'ATCNN', 'Average')
    print('For MS-ATCNN UTI, average throughput is:', results1[0])
    
    results2 = start_simualtion(net, eng, Trained_Net, M, opt_mode, 'Fixed', 'ATCNN', 'Average')
    print('For ATCNN@AverUTI, average throughput is:', results2[0])
    
    results3 = start_simualtion(net, eng, Trained_Net, M, opt_mode, 'Fixed', 'GT', 'Average')
    print('For GT@AverUTI UTI without delay, average throughput is:', results3[0])
    
    results4 = start_simualtion(net, eng, Trained_Net, M, opt_mode, 'Fixed', 'GT', 'Average', 'runtime')
    print('For GT@AverUTI UTI with delay, average throughput is:', results4[0])
    
    results5 = start_simualtion(net, eng, Trained_Net, M, opt_mode, 'Fixed', 'SSS', 'Average')
    print('For SSS@AverUTI, average throughput is:', results5[0])
    
    toc = time.time()
    
    log = [i, net.UE_num, net.Aver_Velocity, results1[0], results2[0],  results3[0], results4[0], results5[0], toc-tic]
    
    csv.update(log, 'result/Fig2_Thr_vs_UEnum_v1.csv')
