...
file_name = 'dataset/Optimal_UTI1/16AP_dataset_UTI_100UE_new_Type1.xlsx'
...
# Target inputs
Tar_inputs = df.iloc[:, [4, 5, 6]].values.tolist()
# Delay outputs
...
############
model.save('trained_model/NN_UTI_100UE_removalV.h5')

for i in range(200):
    log = [i, train_loss[i], test_loss[i]]
    csv.update(log, 'result/AblationStudy_Loss_removalV.csv')
...
loaded_model = tf.keras.models.load_model('trained_model/NN_UTI_100UE_new_Type1.h5')
# revise normalization parameters accordingly here
# Type 1
max_tar_SNR = 20.90560832
min_tar_SNR = 4.433023854
df = shuffled_df[2000:2300]
# Type2
...
############## Ablation study for 4 inputs: ########################
loaded_model = tf.keras.models.load_model('trained_model/NN_UTI_100UE_removalV.h5')
# Target inputs
Tar_inputs = df.iloc[:, [4, 5, 7]].values.tolist()
# Delay outputs
real_outputs = df.iloc[:, [16]].values.tolist()
...
print('Real output should be:', real_outputs)
# errors
RemovalSNR_errors = [x[0] - y for x, y in zip(real_outputs, pre_output_list)]

with open('result/AblationStudy_UTI_errors_removalV.csv', 'w', newline='') as file:
    # Step 4: Using csv.writer to write the list to the CSV file
    writer = csv.writer(file)
    writer.writerow(MS_errors) # Use writerow for single list
    writer.writerow(RemovalSNR_errors)
