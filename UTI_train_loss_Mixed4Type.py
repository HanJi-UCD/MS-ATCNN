...
file_name1 = 'dataset/Optimal_UTI1/16AP_dataset_UTI_100UE_new_Type1.xlsx'
file_name2 = 'dataset/Optimal_UTI2/16AP_dataset_UTI_100UE_new_Type2.xlsx'
file_name3 = 'dataset/Optimal_UTI3/16AP_dataset_UTI_100UE_new_Type3.xlsx'
file_name4 = 'dataset/Optimal_UTI4/16AP_dataset_UTI_100UE_new_Type4.xlsx'

df1 = pd.read_excel(file_name1, dtype={'R_targetAP': 'object', 'SNR_Connected_targetAP': 'object'})
df2 = pd.read_excel(file_name2, dtype={'R_targetAP': 'object', 'SNR_Connected_targetAP': 'object'})
df3 = pd.read_excel(file_name3, dtype={'R_targetAP': 'object', 'SNR_Connected_targetAP': 'object'})
df4 = pd.read_excel(file_name4, dtype={'R_targetAP': 'object', 'SNR_Connected_targetAP': 'object'})
df = pd.concat([df1[0:1800], df2[0:1800], df3[0:1800], df4[0:1800]], ignore_index=True)

df = df[df['V_Tar(m/s)']!=0] # remove 0m/s samples as theta is None 

shuffled_df = df.sample(frac=1)

df = shuffled_df[0:6000]
...
history = model.fit(X_train, y_train, epochs=epoch_num, verbose=0, validation_data=(X_test, y_test),
...
for i in range(epoch_num):
    ...
    csv.update(log, 'result/Mixed4Type_Loss.csv')

############
model.save('trained_model/NN_UTI_100UE_MixedType.h5')
...
loaded_model = tf.keras.models.load_model('trained_model/NN_UTI_100UE_MixedType.h5')
